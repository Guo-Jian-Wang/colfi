...
class SimObservations(SimParameters):
    """Simulate training set.
...
class SimMultiObservations(SimObservations):
...
class AddGaussianNoise(object):
    ...
    def obs_noise(self, measurement, obs_error=None, cholesky_factor=None):
    ...
    def singleNormalObs(self, error_factor=1):
        if self.is_list:
            noisy_sp = []
    ...
    def multiNormalObs(self, factor_sigma=0.2):
        if self.is_list:
            noisy_sp = []
            for i in range(len(self.obs)):
