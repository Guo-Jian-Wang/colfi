...
def get_CovMatrix(chain, params_n, best_values=None):
    ...
    ----------
    chain : array-like
        An ANN or MCMC chain with shape (N, M), where N is the number of chain 
...
class Chains(object):
    ...
    def bestFit(chain, best_type='mode', out_sigma=1, symmetry_error=True, 
        ...
        best_type : str, optional
            The type of the best values of parameters, 'mode' or 'median'. 
            If 'mode', it will take the mode as the best value. 
        ...
        """
        if best_type=='mode':
            best_values = Chains.best_mode(chain, bins=bins, smooth=smooth)
    ...
    def param_devs(chain_1, chain_2):
        ...
        dev = np.array([round(i, 4) for i in dev])
        return dev
    ...
    def cov_matrix(chain, max_error=True, expand_factor=0):
        ...
        max_error : bool, optional
            If True, the diagonal elements of the covariance matrix will be replaced by the 
            estimated maximum errors, which is useful for non-Gaussian distribution.
            If it is set to False, the obtained covariance matrix may be non-positive definite
            for some cases, so it is recommended to put it to True. Default: True
        expand_factor : float, optional
            The expansion factor that is used to expand the error (the standard deviation) 
            of each cosmological parameter. For example, if expand_factor=0.05, 
            the error will has 5% expansion. It only works when max_error is True. Default: 0
        ...
        if max_error:
            ...
            # this will ensure the Pearson correlation coefficient is in the range of [-1,1], 
            # and also ensure covariance matrix is positive definite
...
class UpdateParameterSpace(CheckParameterSpace):
    ...
    chain_0 : None or array-like, optional
        The ANN chain of the (i-1)-th step, where :math:`i\geq2`, if step :math:`\leq2`, 
        ``chain_0`` should be set to None, otherwise, ``chain_0`` should be an array. Default: None
    init_params : None or array-like
        The initial settings of the parameter space. If ``chain_0`` is given, 
        ``init_params`` will be ignored. Default: None
    spaceSigma : int or array-like, optional
        The size of parameter space to be learned. It is a int or a numpy array with 
        shape of (n,), where n is the number of parameters, e.g. for spaceSigma=5, 
    ...
    """
    def __init__(self, step, param_names, chain_1, chain_0=None, init_params=None, 
                 spaceSigma=5, params_dict=None):
    ...
    def small_dev(self, limit_dev=0.01):
        ...
        limit_dev : float, optional
            If the deviation of parameters between two steps smaller than this value, 
            the training process will be over. Default: 0.01 (the deviation < 1%)
        ...
        if max(self.param_devs) <= limit_dev:
            ...
            if self.step==2:
                print('\nThe minimum/maximum parameter deviation between the first step and the initial parameters: ({:.2f}\\sigma, {:.2f}\\sigma)'.format(min(self.param_devs), max(self.param_devs)))
                print('The minimum/maximum relative deviation of parameter errors between the first step and the initial parameters: ({:.2f}%, {:.2f}%)'.format(min(self.error_devs)*100, max(self.error_devs)*100))
            else:
                print('\nThe minimum/maximum parameter deviation between the above two steps: ({:.2f}\\sigma, {:.2f}\\sigma)'.format(min(self.param_devs), max(self.param_devs)))
                print('The minimum/maximum relative deviation of parameter errors between the above two steps: ({:.2f}%, {:.2f}%)'.format(min(self.error_devs)*100, max(self.error_devs)*100))
            return False
    
    #to be updated by setting critical_5_err & critical_5???!!!
    #calculate a shift factor for each parameter based on Fig. 16, and use it in the simulation???
    # def _auto_sigma(self, space_sigma, param_dev, error_dev, critical_5=2/np.sqrt(2), critical_10=7/np.sqrt(2)): #old, to be removed
    def _auto_sigma(self, space_sigma, param_dev, error_dev, 
                    critical_5=2/np.sqrt(2), critical_10=7/np.sqrt(2),
                    critical_5_err=0.15, critical_10_err=0.55):
        ...
        # critical_5  = 0.5 #test?
        # critical_5_err = 0.2 #0.5, 0.3, critical_5_err here should be updated?, to make the setting has physical meaning?
        # critical_10  = 1 #test?
        # critical_10_err = 0.4 #1, 0.4
        # raise ValueError('This function should be updated!') #this function is to be updated and removed?
        
        #this should be updated by using a lot of tests
        critical_5 = 0.27; critical_5_err = 0.16 #based on Fig. 16 (the 2\sigma_p point) of the colfi paper
        critical_10 = 2.3; critical_10_err = 0.55 #based on Fig. 16 (the estimated 7\sigma_p point) of the colfi paper
        
        if param_dev<critical_5 and error_dev<critical_5_err:
            sigma = space_sigma #sigma = 5
        elif critical_5<=param_dev<critical_10 or critical_5_err<=error_dev<critical_10_err:
            sigma = 10
        elif param_dev>=critical_10 or error_dev>=critical_10_err:
            sigma = 15
        return sigma
    ...
    def spaceSigma_all(self):
        ...
        for i in range(len(self.param_names)):
            ...
            _sigmas.append(self._auto_sigma(self.spaceSigma[i], self.param_devs[i], self.error_devs[i], critical_5=1/np.sqrt(2), critical_10=6/np.sqrt(2)))
