...
class BatchNorm(object):
    ...
    def _batchnorm3d(self, name, out_channel):
        ...
        # self.seq.add_module(name, nn.LayerNorm([out_channel]+list(self.sides[-1]), eps=self.eps, elementwise_affine=True))
        # self.seq.add_module(name, nn.InstanceNorm3d(out_channel, eps=self.eps, momentum=self.momentum, affine=True, track_running_stats=True)) #test, not good
        # self.seq.add_module(name, nn.GroupNorm(4, out_channel, eps=self.eps)) #test, not good
...
class Pooling(object):
    ...
    def _pool_outSize(self, pool_name, in_size, padding=0, kernel_size=2, stride=2):
        if pool_name=='maxPool1d' or pool_name=='maxPool2d' or pool_name=='maxPool3d':
            dilation = 1
            out_size = np.floor((in_size + 2*padding - dilation*(kernel_size-1) - 1) / stride + 1).astype(int)
        elif pool_name=='avgPool1d' or pool_name=='avgPool2d' or pool_name=='avgPool3d':
            out_size = np.floor((in_size + 2*padding - kernel_size) / stride + 1).astype(int)
        return out_size
